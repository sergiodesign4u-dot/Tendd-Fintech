...
Every visual decision downstream must trace to a line here (an attribute pair or the taste), or to `design/concept/docs/references.md`. A color, form, or font with no pair here is an invention, not a decision.
...
- **Source:** J1 activate without anxiety (jtbd.md J1); Emma's trust trigger, "explicit plain-language explanation of what the product reads and why" (personas.md); benchmark.md C2 and Mechanism 2 (a specific claim beats vague reassurance).
...
Three contrasting directions were built live in `design/concept/directions.html`, each a different answer to the same brief, each on a light canvas, each Restrained, none using red, a dark canvas, a mint/sage/beige reflex palette, or AI-slop. Each was labeled with the attributes it leads with.
...
The chosen layout is realized in `design/concept/concept.html` (the language stand) and
carried onto the two colored anchor screens in `design/` (Home and Subscription
