...
| **U8** | **One `.trust` per screen, and only where a figure came from the bank.** It is the read-only declaration, not decoration | composition + context | the counter: **5 screens, never twice** (recounted 2026-08-19 by `counts.cjs`; History and trends took one that evening, being the only screen in the product with figures and no source line) + conventions.md section 5 ("the read-only line wherever a trust moment appears") | trust-block | count, and ask where the figures came from |
| **U9** | **One `.total` per screen.** The most important number is the biggest thing on the screen, and two of them means neither is | composition | the counter: **5 screens, never twice** (recounted 2026-08-19 by `counts.cjs`; it was 4 until History and trends took the figure, on the founder's reading that a data screen with no big number "смотрится как-то просто") + design principle 2 in `CLAUDE.md` | big-total | count |
...
| **U12** | **A state change is colour over `--dur-state`, and a component declares a transition only where its host is not a native interactive element.** `base.css` puts the one transition on `:where(a, button, input, select, textarea, summary, label)` and moves four properties: `background-color`, `border-color`, `color`, `text-decoration-color`. Nothing on that list moves an object, and the focus ring is deliberately off it | context | **stage 11, 2026-08-16**, and the census that opened it: 68 state rules across 28 component files, 26 of them with no motion of any kind, so every hover in the app was a hard cut + founder's decision of the same day + the accessibility ground for the ring exclusion (a keyboard user moves faster than 150ms per stop, so a faded ring is always behind the caret) | base.css, and **three components that declare it themselves because their host is not a native element**: plan-option (`div` in all 6 placements), preset-tile (`button` 6 times, `span` 10), alert-item (`a` 8 times, `div` 4). All three read `--dur-state` and `--ease-state`, so none can drift | does the hover host answer to the element list? then nothing to write. Does it not? then one `transition` in the component file, reading the two tokens and never a literal |
| **U13** | **Four verbs, one distance, and nothing travels two.** **Fade** is U12, colour over `--dur-state`. **Advance**: a direction cue moves toward where it points when its target is hovered - the step-forward arrow down, the nav row chevron, the door's pick line and the alert's go line right. **Lift**: an identity mark rises and grows to 1.06 when the target it stands in is hovered - the merchant mark in a row, an alert, a tile or a door, and the destination icon in a tab - **and, since 2026-08-16, the button itself**, up one nudge and no scale, because it is the object you are about to push. On a lifted button the press lands it BACK ON THE SURFACE rather than below the line, so the two halves are one gesture and every neighbouring state is exactly one nudge apart. That half sits behind `@media (hover: hover)`, a pointer query and not a width query: on a touch screen and at the keyboard the plain press stays as it shipped. **Press**: anything pushable goes down while it is held, over `--dur-press`. The distance is `--nudge`, **2px**, everywhere, and the one exception is the checkbox, where a 20px box scales to 0.94 instead because it cannot travel 2px without looking dislodged | context | **founder, 2026-08-16**, on the colour-only pass: "большинство анимаций я не вижу". The three additions answer it, and the single distance is what stops the answer becoming decoration. The exclusions each carry their own ground: a **disabled** control never moves (it would answer a press the product will not honour), a **loading** form never moves (the skeleton's own file has carried that rule in words since stage 07), the **current tab** never moves (it would offer a journey that does not exist), a **figure** never moves (a number that moves is a number you re-read), and a **card never lifts** (a lift needs a shadow, and the Flat Paper Rule spends the only one elsewhere) | logo, step-forward-link, nav-row, door, button, checkbox, tab-bar, range-picker, save-focus-candidate, alert-item, subscription-row, preset-tile | does the object have a direction, an identity or a press? then one verb, one nudge, from the tokens. Does it have none of the three? then it fades and nothing else |
| **U14** | **The brand moves once on arrival, and it answers a pointer anywhere: one crop number, two rules.** The mark's window opens to the whole letter and closes into crop A over `--dur-signature` on `--ease-settle`, and the wordmark's last letter takes petrol as it lands. It runs on the PUBLIC page's bar only, once per load. **the 54 app pages that carry a lockup do not carry `.signature` and must not.** The **hover** is the same gesture PLAYED, and it is NOT fenced: on every lockup in the product, `.lockup:hover` runs the window out to `--crop-whole` and back into crop A over `--dur-signature`, holding no pose at either end. A held crop was tried first and the founder rejected it on sight: the mark's field is the ground behind it, so any part-open crop shrinks the letter off the frame and the rounded square stops being one. The constant is a local custom property on `.brand` with exactly two consumers, and the two animations sit in **two slots of the animation list** through a second local property, `--sig-name` - sharing one slot replayed the arrival every time a pointer left the public page's lockup. | context | **founder, 2026-08-16**: "давай на логотип сделаем анимашку, у нас где-то было расписано как делать анимацию", and it was - `design/concept/logo-crop.html`, the chosen direction, whose whole sentence is "the tighter the crop, the more abstract the mark". That page demonstrates it as an eight second loop; a loop in a bar a person opens fifty times a day is the opposite of this product's first principle, so what ships is one move on arrival. The hover half is the **same founder the next morning**: "при ховере я не вижу анимации". The mark was not a target and still is not one; what answers is the window rather than the mark, because a mark that rose and grew would be a lift and a lift says the object is pressable | brand-mark, brand-wordmark | is this the app? then no `.signature` on the lockup. Is it the public bar? then exactly one, and no second carrier anywhere. Adding a crop value of any kind? then it is a share of `--crop-whole` and never a second number, and the animation that spends it fills `backwards` rather than `both`, or it outranks the hover and kills it |
| **U15** | **One content measure per screen, and every state of a screen ends in the same place.** A block narrows itself only for READING - a paragraph at `--container-text`, a prose card or a form column at 560, a list column at 620 - because that limit is a property of the TEXT. A block that is not text and wants to be narrower is not asking for a measure, it is telling you the screen is too wide, and the answer goes on the pane (`padding-inline` on `.screen`) rather than on the block. **An interruption is the one state measured against something else**: `.screen.interruption` outside the onboarding chain is one message and one way out, it takes `--container-page` on every screen it appears on, and it does not become a dashboard's width because the screen it replaces is one. It is checked against the other interruptions instead, which is the stricter test | composition | **founder, 2026-08-19**, on History and trends at a 1920 window: "что то растянуто на всю ширину что то нет ... я бы хотел чтобы это выглядело круто, а не дёшево". Measured on that one screen, the direct children ended at **1280, 780, 525, 459 and 384** + the counter, `design/kit/screens/edges13.cjs`, over all 54 app screens at 1920: **12 families, 0 split, 1 declared exception** (`upgrade` takes 59rem for its plan row and its three states do not) and **2 interruptions, 1 width** + the same repair made on Home on 2026-08-13, where a cap in `groups-column-set.css` fought the pane and lost. The rule was rewritten the same evening: the founder's answer to the five edges was not the narrow measure it first got but Home's wide one, and the value moved while the rule stood | app-shell, chart-placeholder, category-bars, card | run `edges13.cjs`. Then read the screen's own row: one width under "the pane gives", and everything under "declared its own" is text or a control |
...
| **U18** | **A tab destination carries the account link and never a back control; a screen below one carries the back control and never the account link.** The destinations are **Home, Trends, Alerts and You**: four since 2026-08-21, when the Save tab was retired because it landed on a screen 84 per cent identical to Home and nothing but the bar linked to it. This row said five until 2026-08-23. A back control on a destination offers to leave a place the person did not arrive at from anywhere in particular, and the way out of a destination is the tab bar it already has | context | **founder, 2026-08-19**: "мне не нравится что у нас в трендс есть кнопка назад когда это стало отдельным пунктом" + the counter over both corpora: **16 destination pages, 12 pages below one, 0 wrong in either direction** once History and trends was fixed. That count was taken on 2026-08-19, on the corpus that still carried the Save tab, and it keeps that corpus rather than being guessed forward. It had been the only destination with a back control since 2026-08-18, when Trends became a tab and the bar it was given as a DETAIL screen was never looked at again | app-bar, tab-bar | is the screen one of the four destinations? then the account link and no back. Is it below one? then the back control and no account link |
| **U19** | **The plan chip is a child of the account link, and it appears wherever that link does.** A plan is a fact about the ACCOUNT, so it belongs to the control that opens the account: it lands on Settings, where a plan can be read and changed, and never on the upgrade screen, which is what keeps it a statement rather than a sell. Where there is no account link - the four Upgrade screens - it stays a sibling of the lockup and takes the `margin-left: auto` chip.css gives it. **Inside the link it is pushed by the FORM of the bar and not by the page**, 2026-08-20: glued to the name in the row, where the link is already the trailing slot, and pushed to the far edge in the rail, where the link is stretched to the rail's own width | composition + context | **founder, 2026-08-19**: "Free должно быть везде а не только в трендс", overruling the narrower answer shipped an hour earlier + the counter over both corpora: **15 pages carry the account link and all 15 carry the chip inside it; 4 carry it as a sibling; 0 carry it beside the greeting.** The narrower answer and its argument are kept in `docs/decisions.md`, because a rejected reason that turns out to be overruled is worth more than a rule with no history | app-bar, chip | is there an account link? then the chip is inside it. Is there none? then it is a sibling of the lockup |
