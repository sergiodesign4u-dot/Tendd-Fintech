...
| Variable or class | Was | Became | Why |
|---|---|---|---|
| the type scale | **21 sizes, no variable for any of them**, five of them half steps (14.5, 13.5, 12.5, 11.5, 10.5) | **8 steps at the fold, 10 today**: `--type-display` 46, `--type-figure` 32, `--type-head` 24, `--type-title` 20, `--type-sub` 16, `--type-body` 14, `--type-meta` 12, `--type-label` 10, plus **`--type-step` 40, added 2026-08-15** for the landing's three ordinals and **`--type-section` 32, added 2026-08-16** for a section heading on the public page past the tablet point. That last one is the same VALUE as `--type-figure` and a token of its own, which is rule 4 of this system: two roles, two tokens, even at one value today. `--type-hero` is not counted in either number and never was, because it is a clamp rather than a step | Founder: no half steps. The half steps existed for no stated reason and no two of them were ever compared. **The units became `rem` on 2026-08-13** (2.875 / 2 / 1.5 / 1.25 / 1 / 0.875 / 0.75 / 0.625), which changes nothing in this table: the numbers above are what every step renders at the browser default, and the swap moved zero of 8 596 element records on all 32 coloured screens. See `docs/decisions.md`, the entry of that date |
...
| `button.css`, the label | wrapped whenever its container was narrow | `white-space: nowrap` | Founder finding on the etalon page, where "Start Tendd Pro" folded onto three lines inside a narrow table cell. **Zero pixels move:** all 104 buttons on the 28 coloured pages measure exactly 44px, so none is wrapping today and none is held narrower than its label. What the line removes is the luck. Stage 07 paid for its absence once, widening the onboarding column from 620 to 760 because the plan button wrapped; that was the layout covering for the component. **One exception since 2026-08-13:** `.app .plan-opt .btn` takes `white-space: normal`. The rule assumes a label can be authored once and measured once, and a `rem` type scale hands the text size to the reader, so no label fits every root. Measured: the plan button is 249px at the default and 356px at a 24px root, in a 358px track, and the page scrolled sideways by 16px |
...
| Rule | Reads |
|---|---|
| the type scale | the after value is one of the declared steps (eight at the fold, ten since `--type-step` and `--type-section`) |
