...
- **The free tier is gated by depth and history, never by the core relief experience, by subscription count, or by the number of bank connections.** Traces D3 explicitly, and supersedes the older CLAUDE.md "up to 10 subscriptions" and "up to 2 bank accounts" lines, both since removed (recorded in the Entities scope note). Reason: D3 is the later locked decision and the source of truth, and a connection cap is really a visibility cap.
...
3. (P) What Pro unlocks: history and trends, advanced alerts (trial ending, unusual, duplicate), full step-by-step cancel guides and one-tap cancel links, export. Traces D3. (Connecting more banks is not a Pro unlock: Free is uncapped on connections.)
...
- **Locked (draft, 2026-07-03):**
  ...
  - Free is gated by depth and history, never by subscription count or connection count (D3 over the old CLAUDE.md lines).
