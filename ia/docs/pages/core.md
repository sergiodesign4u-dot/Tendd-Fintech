...
- **FLAG 1 resolved: the Save tab lands on Home in a "save-focus" state, not a new screen.** *(Superseded 2026-08-21: there is no Save tab. The nudge below, block 4, is the answer that survived; see `../nodes/2-6-home.md`.)* The skeleton is locked, so no Save screen is added. Concretely, tapping Save opens Home with three differences from the default: (1) the "N you might not use" cancel-candidates group is pinned to the top and expanded, (2) GC3 Recurring Summary Strip swaps its context line to show potential monthly savings ("You could save up to $32 a month"), and (3) each candidate row shows a "Cancel" affordance inline. It is the same screen, the same data, parameterized. Reason: it honors the lock while giving the Save tab a real, distinct destination Claudia recognizes. Recorded as the resolution of FLAG 1.
