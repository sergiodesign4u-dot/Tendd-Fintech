...
| Design System | [design/kit/overview.html](./design/kit/overview.html) | **In progress, August 2026.** One roadmap row, not two: tokens and components live in the same folder and the same stand, and a row whose page is a section of another row's page is a second entrance to one room. Steps 1 and 2 done. **Step 1** read the flat kit on two instruments, Claude by parse and Codex read-only: 40 variables, one orphan, zero literals past a variable in the stylesheet, and 23 font sizes plus 24 spacings that were never tokenised at all (`docs/tokens-audit.md`). The same sweep measured every control in a browser at two viewports, 594 records, and found `Try again` playing in three weights on four error screens (`docs/census.md`). **Step 2** consolidated the families by anatomy: **68 components became 55**, with a renaming map that step 6 executes and stage 12 reads. The component registry is `design/kit/_nav.js`, which draws both the hub cards and the side panel |
