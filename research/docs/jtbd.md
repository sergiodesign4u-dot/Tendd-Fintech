...
**Built on:** research/docs/personas.md, research/docs/research.md (v_refresh), research/docs/strategy.md, research/docs/ux-patterns.md.
...
- Research basis: strategy.md Segment A JTBD (confirmed); research.md key conclusion 1; competitors.md gap G1 (calm, focused subscription tracker is the unoccupied position).
...
- What makes it different from competitors: no competitor has designed the first subscription view as a calm emotional experience. Every competitor leads with a dashboard, a budget, or a bank connection request. (competitors.md P1, P2.)
...
- Research basis: Every bank-connected competitor asks for bank access in step 1 (competitors.md P1). 68% of users are uncomfortable with apps' access to financial data (O16, personas.md, from bankingdive.com). 4 in 5 users did not know apps store bank usernames and passwords (O17).
...
- Product consequence: two explicit activation paths (bank-connected and privacy-first) must both converge on the same aha moment. The trust job must be resolved in onboarding before the main job becomes accessible. (research.md section 2, strategy.md section 3 O1.)
- Competitors that close this job partially: Apple Card (benchmark C8 = 4/5, shows value before commitment). No direct competitor closes it fully. (benchmark.md.)
...
- Emotional peak: the cancel-and-save moment is the most emotionally powerful moment in the product. "After it saved me $400 in 15 minutes, I'd say it is." (O from personas.md Claudia, from finance.yahoo.com.) No competitor has designed this moment as a positive emotional experience (competitors.md gap G4).
- Product consequence: cancel guides and direct cancel links are a Pro feature. The upgrade trigger lives here. (research.md AARRR Revenue stage.)
...
- Research basis: Apple Card mechanism M3 from research.md (real merchant names and logos replace cryptic bank codes). N26's EU merchant cleanup validates this as table stakes (strategy.md J5, citing support.apple.com/en-us/102329 and n26.com).
...
- Research basis: strategy.md J4 (confirmed); ux-patterns.md BP3 (action on surprise - users act when notified, not when reminded to review). Monzo principle M1 from research.md: active voice in financial notifications ("we couldn't process your payment" not "payment declined").
...
- Research basis: strategy.md J6 (new, confirmed by ReSubs). ReSubs built 18,000+ users and 4.5 stars Google Play entirely on a no-bank-connection model (O8, personas.md). 68% of users uncomfortable with app access to bank data (O16). The privacy-or-convenience binary is a confirmed market gap (competitors.md P3).
- Product consequence: manual entry + AI screenshot import + email receipt parsing must be a first-class activation path with equal visual weight to bank connection. The privacy path is not a fallback - it is a primary path for a confirmed segment. (research.md AARRR Activation MVP decision.)
...
- Research basis: "Getting rid of those felt like a little pay raise" (O7, personas.md, from checkthat.ai). "I was shocked when I entered all my subscriptions and saw the yearly total" (O15, personas.md, from 19pine.ai - this shock is positive). The cancel-and-save moment is identified as the most shareable moment in the product (research.md key conclusion 3).
- Product consequence: the cancel-and-save moment must be designed explicitly - not a generic success toast but a moment with specific numbers ("you just freed up $14.99/month, that's $179.88/year"). This is also where the share prompt lives (research.md AARRR Referral MVP decision).
...
- Research basis: over half of consumers prefer built-in app settings to manage data permissions (O20, personas.md, from bankingdive.com). 4 in 5 users did not know apps store their bank credentials (O17). Monarch Money makes "we do not sell your data" a front-page trust signal (research.md D3 gap - no subscription tracker does this prominently).
...
- Research basis: research.md H2 (Spotify Wrapped-style share is a hypothesis). Rocket Money's top testimonials all reference this moment - users tell friends. (research.md AARRR Referral stage; competitors.md gap #4.) The specific format (subscription count + total spend, no bank data visible) is the privacy-safe shareable card.
- Note: this is a hypothesis about virality (marked [? hypothesis] in research.md). Real share rates are unknown. The product must design for shareability but cannot count on it.
...
- Basis: strategy.md Segment C (later stage). Emma and Monarch both added shared modes (research.md). Validated as a real need but confirmed as too complex for MVP. This job is out of scope.
...
| E3: Feel safe and in control of data | 2 | 3 | 1 | One-tap data deletion + plain-language data explanation on first screen | Monarch Money mentions data non-sale (research.md D3). No subscription tracker makes this front-page. |
...
- **Bill negotiation (percentage fee):** Closes J2 but with a revenue model that misaligns incentives (research.md D1). Cut from business model entirely, not just MVP.
...
| J4 notifications build trust if designed like Monzo's active-voice principle | Hypothesis - consistent with Monzo benchmark (research.md M1) but not tested for this product | [?] Remains hypothesis. No new evidence. |
